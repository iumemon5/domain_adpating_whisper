...
class DataCollatorSpeechSeq2SeqDomain:
    ...
    def __call__(self, features):
        ...
        label_texts = []
        for f in features:
            domain = f.get("domain","normal")   # your dataset should add "domain" field
...
def build_trainer(cfg: Config) -> Trainer:
    ...
    for block in model.model.encoder.layers[-4:]:
        for p in block.parameters():
            p.requires_grad = True

    # unfreeze last 4 decoder blocks (attention + MLP only)
    for block in model.model.decoder.layers[-4:]:
...
def parse_args() -> Config:
    ...
    p.add_argument("--dataset-root", type=str, default="/home/braindeck/ssd/irfan/dataset/dyslexia_dataset_webdataset",
    ...
    # =============================================================================
    # Run identification
    p.add_argument("--output-dir", type=str, default="runs/whisper_dyslexia_domain_adaptation",
                   help="Directory to save training outputs")
    p.add_argument("--run-name", type=str, default="whisper_dyslexia_domain_adaptation_r2",
    ...
    p.add_argument("--num-train-epochs", type=int, default=500,
