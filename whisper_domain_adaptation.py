...
class Config:
    ...
    # Domain Balancing
    # =============================================================================
    # Oversample dyslexic domain at the shard level: duplicate dyslexic shard URLs
    # by this integer factor (1 = no oversampling). If balance_domains is True,
    # this factor is used as a minimum and may be increased to approximately
    # match normal-domain sample counts.
    dyslexic_oversample_factor: int = 1
    # If True, compute an integer oversample factor so that dyslexic and normal
    # domains have roughly equal estimated sample counts (based on metadata).
    balance_domains: bool = False
    # Apply per-sample loss weighting by domain during training. When enabled,
    # losses from dyslexic samples are multiplied by dyslexic_loss_weight.
    use_loss_weighting: bool = False
    dyslexic_loss_weight: float = 1.0
...
def infer_domain_label(sample: Dict[str, Any]) -> str:
    ...
    if "train_normal" in url or "val_normal" in url or key.startswith("train-") or key.startswith("test-"):
        return "normal"
    if "train/" in url or "val_small" or "val" in url:
        return "dyslexic"
...
def build_datasets(cfg: Config, processor: WhisperProcessor) -> Tuple[Any, Any, int, int]:
    ...
    val_domains = [
        ...
        ("dyslexic", os.path.join(cfg.dataset_root, "val")),
    ...
    # Optional: oversample dyslexic shards to balance domains
    ...
    if cfg.balance_domains and dys_count > 0 and norm_count > 0:
        ...
        target = int(_math.ceil(norm_count / max(1, dys_count)))
        oversample_factor = max(oversample_factor, target)

    if oversample_factor > 1 and len(dys_urls) > 0:
        print(
    ...
    # Recompute train_urls after potential oversampling
    train_urls = [url for urls in train_domain_urls.values() for url in urls]
...
class DataCollatorSpeechSeq2SeqDomain:
    ...
    def __call__(self, features):
        ...
        # Domain IDs for loss-weighting (1=dyslexic, 0=normal)
        if self.include_domain_ids:
            domain_ids = torch.tensor(
                [1 if f.get("domain", "normal") == "dyslexic" else 0 for f in features],
                dtype=torch.float32,
            )
...
def build_metrics(processor: WhisperProcessor):
    ...
    def compute_metrics(pred):
        ...
        pred_ids = to_int64(pred_ids)
        pred_ids = torch.clamp(pred_ids, min=0, max=vocab_size - 1)
        pred_ids = to_list(pred_ids)
        label_ids = to_int64(label_ids)
        pad_token_tensor = torch.tensor(
            pad_id,
            dtype=torch.int64,
            device=label_ids.device,
        )
        label_ids = torch.where(label_ids != -100, label_ids, pad_token_tensor)
        label_ids = torch.clamp(label_ids, min=0, max=vocab_size - 1)
        label_ids = to_list(label_ids)

        # Decode with special tokens to extract domain from labels
        label_str_raw = processor.batch_decode(label_ids, skip_special_tokens=False)

        # Decode without special tokens for CER calculation
        ...
        # Overall CER
        metrics = {"cer": cer_metric.compute(predictions=pred_str, references=label_str)}

        # Group predictions and labels by domain
        domain_preds = {"dyslexic": [], "normal": []}
        domain_labels = {"dyslexic": [], "normal": []}

        for i, raw_label in enumerate(label_str_raw):
            domain = "normal"  # Default domain
            if "<|domain:dyslexic|>" in raw_label:
                domain = "dyslexic"
            
            domain_preds[domain].append(pred_str[i])
            domain_labels[domain].append(label_str[i])

        # Compute per-domain CER
        for domain in ["dyslexic", "normal"]:
            if domain_labels[domain]:
                domain_cer = cer_metric.compute(
                    predictions=domain_preds[domain],
                    references=domain_labels[domain],
                )
                metrics[f"cer_{domain}"] = domain_cer
...
class DomainSeq2SeqTrainer(Seq2SeqTrainer):
    """Seq2SeqTrainer variant that strips domain metadata and guards generation."""
    ...
    def prediction_step(
        ...
        # Prepare and sanitize inputs (remove fields unsupported by generate)
        if isinstance(inputs, dict):
            inputs = dict(inputs)
            inputs.pop("domain_ids", None)

        inputs = self._prepare_inputs(inputs)

        # Ensure domain_ids is fully removed after device placement
        if isinstance(inputs, dict):
            inputs.pop("domain_ids", None)
        else:
            try:
        ...
        # Convert to plain dict for downstream merges
        inputs = {k: v for k, v in inputs.items() if k != "domain_ids"}

        has_labels = "labels" in inputs
        labels = inputs.pop("labels") if has_labels else None

        # Optionally compute loss using labels
        loss = None
        if has_labels and not prediction_loss_only:
            with torch.no_grad():
                model_inputs = {**inputs, "labels": labels}
                model_inputs.pop("domain_ids", None)
        ...
        if not self.args.predict_with_generate:
            # Fall back to base behavior without generation
            return (loss, None, labels)

        # Build generation inputs: exclude decoder_input_ids and any stray unsupported keys
        ...
        with torch.no_grad():
            try:
                ...
                if "seek_sequence" not in str(exc):
                    raise
                ...
                warnings.warn(
                    "Whisper generation returned an empty sequence; injecting EOS to continue evaluation.",
                    RuntimeWarning,
                )
...
class WeightedSeq2SeqTrainer(DomainSeq2SeqTrainer):
    """Seq2SeqTrainer with per-sample loss weighting by domain.

    Expects a float tensor `domain_ids` in inputs of shape [batch], where 1.0
    marks dyslexic samples and 0.0 marks normal samples. Multiplies the per-sample
    loss by (1 + (w-1)*domain_id), where w is `dyslexic_loss_weight`.
    """

    def __init__(self, *args, dyslexic_loss_weight: float = 1.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.dyslexic_loss_weight = float(dyslexic_loss_weight)

    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):  # type: ignore[override]
        # Extract and remove domain_ids before passing to model
        domain_ids = inputs.pop("domain_ids", None)
        labels = inputs.get("labels")
        outputs = model(**inputs)

        if labels is None:
            loss = outputs["loss"] if isinstance(outputs, dict) and "loss" in outputs else outputs.loss
            return (loss, outputs) if return_outputs else loss

        logits = outputs.get("logits")
        vocab = logits.size(-1)
        loss_fct = torch.nn.CrossEntropyLoss(reduction="none", ignore_index=-100)
        loss_per_token = loss_fct(logits.view(-1, vocab), labels.view(-1))
        loss_per_token = loss_per_token.view(labels.size())
        mask = (labels != -100).float()
        per_sample = (loss_per_token * mask).sum(dim=1) / (mask.sum(dim=1) + 1e-8)

        if domain_ids is not None and self.dyslexic_loss_weight != 1.0:
            domain_ids = domain_ids.to(per_sample.device)
            weights = 1.0 + (self.dyslexic_loss_weight - 1.0) * domain_ids
...
def build_trainer(cfg: Config) -> Seq2SeqTrainer:
    ...
    include_domain_ids = bool(cfg.use_loss_weighting and cfg.dyslexic_loss_weight != 1.0)
    data_collator = DataCollatorSpeechSeq2SeqDomain(
        processor=processor,
        include_domain_ids=include_domain_ids,
    )
    ...
    decoder_prompt_ids = processor.get_decoder_prompt_ids(language="korean", task="transcribe")
    model.generation_config.no_repeat_ngram_size = 0
    model.generation_config.num_beams = 3
    model.generation_config.length_penalty = 1.0
    model.generation_config.max_new_tokens = 128 # Added max_new_tokens
    ...
    # If we're using loss weighting, disable label smoothing here so the custom
    # per-sample loss matches expectations.
    label_smoothing = 0.0 if cfg.use_loss_weighting else 0.1
    ...
    if cfg.use_loss_weighting and cfg.dyslexic_loss_weight != 1.0:
        print(f"[balance] Using loss weighting: dyslexic_loss_weight={cfg.dyslexic_loss_weight}")
        trainer = WeightedSeq2SeqTrainer(
            ...
            dyslexic_loss_weight=cfg.dyslexic_loss_weight,
        )
    else:
        trainer = DomainSeq2SeqTrainer(
            model=model,
            args=args,
            train_dataset=train_dataset,
            eval_dataset=val_dataset,
...
def parse_args() -> Config:
    ...
    p.add_argument("--num-train-epochs", type=int, default=200,
    ...
    p.add_argument("--balance-domains", action="store_true",
                   help="Automatically oversample dyslexic shards to roughly match normal-domain sample count")
    p.add_argument("--use-loss-weighting", action="store_true",
                   help="Enable per-sample loss weighting by domain during training")
    p.add_argument("--dyslexic-loss-weight", type=float, default=1.0,
                   help="Multiplier for dyslexic sample loss when --use-loss-weighting is active")
    ...
    return Config(
        ...
        use_loss_weighting=a.use_loss_weighting,
        dyslexic_loss_weight=a.dyslexic_loss_weight,
